...
function App() {
  ...
  useEffect(() => {
    if (
      formError.cardHolder === "success" &&
      formError.cardNumber === "success" &&
      formError.expMM === "success" &&
      formError.expYY === "success" &&
      formError.CVC === "success"
    ) {
      setAccepts(false);
    }
  }, [formError]);
  ...
  const validation = (values) => {
    ...
    for (let i = 0; i < values.cardHolder.length; i++) {
      ...
      if (
        !digiRegex.test(values.cardHolder[i]) &&
        values.cardHolder.length > 0
      ) {
    ...
    for (let i = 0; i < values.CVC.length; i++) {
      ...
      if (values.CVC.length > 2 && digiRegex.test(values.CVC[i])) {
        errors.CVC = "success";
      }
    }
      ...
      <div className="front-card">
        ...
        <div className="card-numbers">
          {formValues.cardNumber
            ? ` ${slicedCardNumber(0, 4)}  ${slicedCardNumber(
                4,
                8
              )}  ${slicedCardNumber(8, 12)}  ${slicedCardNumber(12, 16)}`
            : "0000 0000 0000 0000"}
        </div>
        <div className="flex-row space-between mobile-details">
          <div className=" mobile-card-name">
            {formValues.cardHolder ? formValues.cardHolder : "Jane Appleseed"}
          </div>
          <div className="mobile-card-date">
            {formValues.expMM || formValues.expYY
              ? `${formValues.expMM}/ ${formValues.expYY}`
              : "00/00"}
          </div>
        </div>
        ...
        <div className="input-box">
          <form className="input-inner" onSubmit={handleSubmit}>
            <label className="flex-column uppercase">
              ...
              <input
                className={
                  formError.cardHolder === "error1"
                    ? "name-input input-style-red"
                    : formError.cardHolder === "error2"
                    ? "name-input input-style-red"
                    : formError.cardHolder === "success"
                    ? "name-input input-style-green"
                    : "name-input input-style"
                }
                type="text"
            ...
            <label className="flex-column uppercase ">
              ...
              <input
                className={
                  formError.cardNumber === "error1"
                    ? "name-input input-style-red"
                    : formError.cardNumber === "error2"
                    ? "name-input input-style-red"
                    : formError.cardNumber === "error3"
                    ? "name-input input-style-red"
                    : formError.cardNumber === "success"
                    ? "name-input input-style-green"
                    : "name-input input-style"
            ...
            <div className="flex-row  gap-20">
              <label className="flex-column uppercase">
                ...
                <div className="flex-row gap-5">
                  <input
                    className={
                      formError.expMM === "error1" ||
                      formError.expMM === "error2"
                        ? "numbers-input input-style-red"
                        : formError.expMM === "success"
                        ? "numbers-input input-style-green"
                        : "numbers-input input-style"
                    }
                    type="text"
                    placeholder="MM"
                    name="expMM"
              ...
              <label className="flex-column uppercase">
                ...
                <input
                  className={
                    formError.CVC === "error1" ||
                    formError.CVC === "error2" ||
                    formError.CVC === "error3"
                      ? "numbers-input width-50 input-style-red"
                      : formError.CVC === "success"
                      ? "numbers-input width-50 input-style-green"
                      : "numbers-input width-50 input-style"
                  }
                  type="text"
                  placeholder="e.g. 123"
                  name="CVC"
